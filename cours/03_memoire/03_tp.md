...
👉 [Ouvrir l'exercice BitReg](https://seed.music-music.fr/)
...
- Si load=0 : Mux sélectionne a=dff_out (maintien)
- Si load=1 : Mux sélectionne b=d (nouvelle valeur)
...
👉 [Ouvrir l'exercice Register16](https://seed.music-music.fr/)
...
👉 [Ouvrir l'exercice PC](https://seed.music-music.fr/)
...
```
  ...
  out ───────►│ Adder   │───► out+1
              └────┬────┘
                   │
              ┌────▼────┐
  out ────────┤ Mux inc │───► val_if_inc
              └────┬────┘
                   │
         in ──┬────▼────┐
              │ Mux load│───► val_if_load
              └────┬────┘
                   │
          0 ──┬────▼────┐
              │Mux reset│───► to_reg
...
```vhdl
-- Incrémenter la valeur courante
...
👉 [Ouvrir l'exercice RAM8](https://seed.music-music.fr/)
...
```vhdl
-- Démultiplexer le signal load vers le bon registre
...
👉 [Ouvrir l'exercice RAM64](https://seed.music-music.fr/)
...
➡️ **Chapitre 04 : Architecture Machine** — Définir l'ISA du CPU !
